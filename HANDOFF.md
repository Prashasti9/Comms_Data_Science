...
**`pinviz`** is a small Python **data-visualization library** built for a
communication course. It makes "communication-first" charts that tell one
data story — **Pinterest's 2025 results** — where every chart is a deliberate
design choice from the **Evergreen Data Visualization Checklist** and maps to
a family in the **Quantitative Chart Chooser**.
...
```
Comms_Data_Science/
├─ src/pinviz/              # THE LIBRARY (this is what's graded)
...
├─ tests/
│  └─ test_pinviz.py        # 9 pytest tests (return types + 2 calculations)
...
│  ├─ pinviz_gallery.py               # renders the 5 charts + a combined PDF
│  ├─ pinviz_poster.py                # the one-page infographic poster
│  ├─ pinviz_animation.py             # animated growth line  → pinviz_growth.gif
│  ├─ pinviz_story_animation.py       # full story animated   → pinviz_story.gif
│  ├─ pinviz_01_big_number.png … 05_arpu_bar.png   # the 5 chart images
│  ├─ pinviz_gallery.pdf              # 5 charts combined
│  ├─ pinviz_poster.png / .pdf        # the poster
│  ├─ pinviz_growth.gif               # animated growth line
│  └─ pinviz_story.gif                # animated full story
├─ README.md                # install + usage + the Chart Chooser mapping table
...
**Install name vs import name:** the package installs as `pinviz` and imports
as `pinviz`. Version is `0.1.0` (first working version).

---

## 3. The library API (the 5 chart functions)

All live in `src/pinviz/core.py`. Each takes a pandas DataFrame (or simple
values) and **returns a matplotlib `Figure`** you can `savefig` in any format.

| Function | Chart Chooser family | What it answers | Key call |
| --- | --- | --- | --- |
| `big_number(value, unit, label, footnote="")` | Single important number | How big is the headline? | `pinviz.big_number(619, "M", "monthly active users")` |
| `growth_line(df, x, y, title, annotate=None)` | Change over time | Which way is the trend going? | `pinviz.growth_line(mau, "year", "maus_millions", title=..., annotate=(2021,"dip"))` |
| `revenue_bar(df, category, value, title, spotlight=None)` | Comparison | Who contributes the most? | `pinviz.revenue_bar(reg, "region", "revenue_musd", title=..., spotlight="US & Canada")` |
| `share_gap(df, category, part_a, part_b, title, highlight=None)` | Parts of a whole | Users vs revenue mismatch | `pinviz.share_gap(reg, "region", "maus_millions", "revenue_musd", title=..., highlight="Rest of World")` |
| `arpu_bar(df, category, revenue, users, title, spotlight=None)` | Comparison / ratio | How well is each region monetised? | `pinviz.arpu_bar(reg, "region", "revenue_musd", "maus_millions", title=...)` |

---

## 4. How the code works (module by module)

### `src/pinviz/theme.py` — one shared design system
...
### `src/pinviz/core.py` — the 5 functions
Each function: builds a figure, draws its chart, applies the shared theme, and
returns the `Figure`. `revenue_bar` and `arpu_bar` share one private helper
`_spotlight_barh` (sorted bars, one highlight colour, end-of-bar labels) so
there's no duplicated code. `arpu_bar` computes revenue ÷ users itself so the
ratio is never hand-entered.

### `src/pinviz/__init__.py` — the public face
Imports the 5 functions so users can call `pinviz.growth_line(...)`, and
defines `__all__` and `__version__`.

### The design rules every chart follows (Evergreen)
...
**Five library charts** (run `python examples/pinviz_gallery.py`):
1. `pinviz_01_big_number.png` — **619M** big-number callout.
2. `pinviz_02_growth_line.png` — MAU growth 2019–2025, 2021 dip annotated.
3. `pinviz_03_revenue_bar.png` — Q4 revenue by region (US & Canada spotlighted).
4. `pinviz_04_share_gap.png` — users-vs-revenue split (Rest of World highlighted).
5. `pinviz_05_arpu_bar.png` — implied revenue per user by region.

**One composed poster** (`python examples/pinviz_poster.py`):
- `pinviz_poster.png` / `.pdf` — a one-page infographic: red kicker → serif
...
- `pinviz_growth.gif` — the growth line drawing in (`pinviz_animation.py`).
- `pinviz_story.gif` — the full story building in sequence
  (`pinviz_story_animation.py`).
...
1. **`ModuleNotFoundError` on `import pinviz`.** Cause: the modern `src/`
...
python -c "import pinviz; print(pinviz.__all__)"   # confirm it imports

python -m pip install -e ".[dev]"       # install pytest too
python -m pytest -q                     # run the 9 tests (should pass)

python examples/pinviz_gallery.py       # render the 5 charts + gallery PDF
python examples/pinviz_poster.py        # render the poster (PNG + PDF)
python examples/pinviz_animation.py     # render pinviz_growth.gif
python examples/pinviz_story_animation.py   # render pinviz_story.gif
...
**One-line summary for the video:** *"pinviz is a small Python library built on
matplotlib and pandas; each of its five functions maps to a Quantitative Chart
Chooser family and follows the Evergreen checklist, telling the story that
Pinterest's users are global but its revenue is domestic."*
